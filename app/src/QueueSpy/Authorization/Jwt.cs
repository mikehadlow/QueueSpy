	...
	public static class JsonWebToken
	{
		...
		public static string Decode(string token, byte[] key, bool verify = true)
		{
			...
			if (verify)
			{
				...
				if (headerData.ContainsKey ("typ") || headerData.ContainsKey ("alg")) {
					...
					throw new SignatureVerificationException(string.Format("Invalid Header. Expected keys: 'typ' and 'alg', got keys: {0}", string.Join(", ", headerData.Keys)));
				}
